...
export class StrategyAdvisorAgent extends BaseAgent {
  async analyze(data: FormValues): Promise<AgentResult> {
    ...
    Your task is to analyze a company's responses to an AI readiness assessment and provide strategic recommendations.
    Focus on their business challenges, priority areas, and timeline expectations.
    
    Provide a score from 0-25 for the Leadership Alignment pillar based on their responses.
    
    IMPORTANT: If you need more information to provide a better assessment, include at most 1 follow-up question.
    
    Format your response as a JSON object with the following structure:
    {
      "insights": "A detailed analysis of the company's strategic alignment for AI",
      "score": 0-25,
    ...
    try {
      ...
      if (result.followUpQuestions && result.followUpQuestions.length > 0) {
        ...
        result.followUpQuestions = result.followUpQuestions.map((q, index) => {
          ...
          const uniqueId = `strategy-advisor-q${index}`;
          return {
            ...q,
            id: uniqueId,
            answered: false,
      ...
      return {
        insights: "Error analyzing strategic alignment",
        score: 0,
        recommendations: [
          "Unable to analyze strategic alignment due to an error",
        ],
      };
    }
  }
  ...
  protected getFollowUpSystemPrompt(): string {
    return `You are an expert Strategy Advisor specializing in AI adoption strategies.
    You've received an answer to a follow-up question you asked about the company's strategic alignment for AI.
    
    Update your analysis based on this new information.
    
    Provide an updated score from 0-25 for the Leadership Alignment pillar based on all information available.
    
    Format your response as a JSON object with the following structure:
    {
      "insights": "A detailed analysis of the company's strategic alignment for AI, incorporating the new information",
      "score": 0-25,
      "recommendations": ["recommendation1", "recommendation2", "recommendation3"]
    }
